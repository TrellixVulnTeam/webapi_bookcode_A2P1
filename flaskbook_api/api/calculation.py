...
def detection(request):
    ...
    try:
        ...
        return jsonify("The model is not fount"),404
    ...
    for box,label,score in zip(output["boxes"],output["labels"],output["scores"]):
        if score >0.6 and labels[label] not in dict_results:
            ...
            dict_results[labels[label]] = round(100*score.items())
    dir_image = str(basedir / "data" / "output"/filename)

    cv2.imwrite(dir_image,cv2.cvtColor(result_image,cv2.COLOR_RGB2RGB))
